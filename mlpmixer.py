...
class MixerBlock(nn.Module):
    ...
    """
    def __init__(
            self, dim, seq_len, mlp_ratio=(0.5, 4.0), activation='gelu', drop=0., drop_path=0.):
...
class MLPMixer(nn.Module):
    ...
    def forward(self, images):
        ...
        """
        patch_embds = self.patchemb(images)      # step1: Go through the patch embedding
        mixer_output = self.blocks(patch_embds)  # step 2 Go through the mixer blocks
        norm_mixer_output = self.norm(mixer_output ) # step 3 go through layer norm
        pool_output = norm_mixer_output.mean(dim=1)
        # step 4 Global averaging spatially
        # Classification
        classif_out = self.head(pool_output)
